...
const SUGGESTIONS = [
  { slug: "plombier-toulouse", name: "Plombier Toulouse", metier: "Plombier", city: "Toulouse", price: 199, img: "plombier-toulouse" },
  { slug: "serrurier-bordeaux", name: "Serrurier Bordeaux", metier: "Serrurier", city: "Bordeaux", price: 179, img: "serrurier-bordeaux" },
  { slug: "electricien-rennes", name: "Électricien Rennes", metier: "Électricien", city: "Rennes", price: 159, img: "electricien-rennes" },
];

/** Sidebar sticky des articles de blog : CTA fixe + suggestions de fiches. */
            ...
            {SUGGESTIONS.map((s) => (
              <li key={s.slug}>
                <Link href={`/fiches-google/${s.slug}`} className="group flex items-center gap-3">
                  <img src={`/assets/listings/${s.img}.png`} alt="" className="size-14 shrink-0 rounded-xl object-cover" />
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 flex items-center gap-1.5 text-xs"><span className="font-bold text-primary">{s.price} €</span><span className="inline-flex text-[#fbbc04]">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className="size-3 fill-current" />)}</span></span>
                  </span>
                </Link>
              </li>
            ))}
