...
export default async function ClientDashboard({ searchParams }: { searchParams: Promise<{ verify?: string; success?: string }> }) {
  const t = await getTranslations("dash");
  const locale = await getLocale();
  const { verify, success } = await searchParams;
    ...
    <div className="mx-auto max-w-[1200px] space-y-6">
      ...
      {success === "1" && (
        <div className="flex items-start gap-3 rounded-xl border border-success/40 bg-success/10 p-4">
          <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-success/20"><CheckCircle2 className="size-5 text-success" /></span>
          <div>
            <p className="font-bold text-success">{t("client.successTitle")}</p>
            <p className="mt-0.5 text-sm text-muted-foreground">{t("client.successText")}</p>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="space-y-5">
          ...
          <Panel className="bg-[linear-gradient(160deg,#eff5ff,#ffffff)]">
            ...
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-2.5 text-sm font-bold text-white hover:opacity-90"><MessageCircle className="size-4" /> {t("client.chatWhatsApp")}</a>
          </Panel>
        </aside>
      </div>
